...
<head>
    ...
    .ph-scale {
        background: linear-gradient(to right, #ff5252, #ffb142, #33d9b2, #34ace0, #706fd3);
    }
    .marker {
        width: 10px;
        height: 25px;
...
<body>
    ...
    <div class="container">
        <div class="box">
            <h2>Fósforo</h2>
            <div class="value">
                <span id="phosphorus-status" class="status off">AUSENTE</span>
            </div>
        </div>
        <div class="box">
            <h2>Potássio</h2>
            <div class="value">
                <span id="potassium-status" class="status off">AUSENTE</span>
            </div>
        </div>
        <div class="box">
            <h2>pH do Solo</h2>
            <div class="value" id="ph-value">0.0</div>
            <div class="scale ph-scale">
                <div id="ph-marker" class="marker" style="left:0%"></div>
            </div>
    ...
    function updateUI(data) {
        ...
        if (data.sensors) {
            // Fósforo
            updateElementIfChanged('phosphorus-status', {
                text: data.sensors.phosphorus ? 'PRESENTE' : 'AUSENTE',
                className: 'status ' + (data.sensors.phosphorus ? 'on' : 'off')
            }, true);

            // Potássio
            updateElementIfChanged('potassium-status', {
                text: data.sensors.potassium ? 'PRESENTE' : 'AUSENTE',
                className: 'status ' + (data.sensors.potassium ? 'on' : 'off')
            }, true);

            // pH
            if (typeof data.sensors.ph === 'number') {
                updateElementIfChanged('ph-value', data.sensors.ph.toFixed(1));

                // Atualiza posição do marcador
                const markerPosition = (data.sensors.ph / 14.0 * 100.0) + '%';
...
void AsyncSoilWebServer::handleData(AsyncWebServerRequest *request) {
    ...
    sensors["ph"] = telemetry.ph;
    sensors["temperature"] = telemetry.temperature;
    sensors["humidity"] = telemetry.humidity;
    sensors["phosphorus"] = telemetry.phosphorusPresent;
    sensors["potassium"] = telemetry.potassiumPresent;
...
String AsyncSoilWebServer::prepareJsonMessage(bool dataOnly) {
    ...
    sensors["phosphorus"] = data.phosphorusPresent;
    sensors["potassium"] = data.potassiumPresent;
